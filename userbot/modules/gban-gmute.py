...
async def muter(rkG):
    ...
    if gmuted2 := is_globelmuted(rkG.sender_id):
        for i in gmuted2:
...
@javes.on(rekcah05(pattern='ungmute(?: |$)(.*)', allow_sudo=True))
...
async def gspider(rk):    
    ...
    try:
      if unglobelmute(user.id) is False:
     	...
     	pass
    return await rkp.edit('`Ungmuted`') 
        

@javes.on(rekcah05(pattern='gban(?: |$)(.*)', allow_sudo=True))
...
@javes.on(rekcah05(pattern='ungban(?: |$)(.*)', allow_sudo=True))
