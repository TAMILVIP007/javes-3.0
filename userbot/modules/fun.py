...
@javes05(outgoing=True, pattern="^!fp$")
async def facepalm(e):
    """ Facepalm  🤦‍♂ """
    await e.edit("🤦‍♂")


@javes05(outgoing=True, pattern="^!cry$")
async def cry(e):
...
@javes05(outgoing=True, pattern="^!cp(?: |$)(.*)")
...
@javes05(outgoing=True, pattern="^!str(?: |$)(.*)")
...
async def zal(zgfy):
    ...
    if not zgfy.text[0].isalpha() and zgfy.text[0] not in ("/", "#", "@", "!"):
        ...
        await zgfy.reply("".join(reply_text))
...
@javes05(outgoing=True, pattern="^!hi$")
async def hoi(hello):
    """ Greet everyone! """
    await hello.edit(choice(HELLOSTR))


@javes05(outgoing=True, pattern="^!owo(?: |$)(.*)")
async def faces(owo):
    """ UwU """
...
@javes05(outgoing=True, pattern="^!iwi(?: |$)(.*)")
...
async def Hmmmmmm(e):
    ...
    for _ in range(15):
        t = t[:-1] + "mm"
        await e.edit(t)
@javes05(outgoing=True, pattern="^Ooo$")
async def Oof(e):
    t = "Oof"
    for _ in range(15):
        t = t[:-1] + "oo"
...
@javes05(outgoing=True, pattern="^!bye$")
async def iqless(e):
    await e.edit("Kek thx bye")       
                      

@javes05(outgoing=True, pattern="^!moon$")
...
@javes05(outgoing=True, pattern="^!earth$")
...
@javes05(outgoing=True, pattern="^!clock$")
...
@javes05(outgoing=True, pattern="^!rain$")
...
@javes05(outgoing=True, pattern="^!love$")
...
@javes05(outgoing=True, pattern="^!clap(?: |$)(.*)")
...
@javes05(outgoing=True, pattern="^!bt$")
async def bluetext(bt_e):
    """ Believe me, you will find this useful. """
    if await bt_e.get_reply_message() and bt_e.is_group:
        await bt_e.edit(
            "/BLUETEXT /MUST /CLICK.\n"
            "/ARE /YOU /A /STUPID /ANIMAL /WHICH /IS /ATTRACTED /TO /COLOURS /kickme /punchme?")


@javes05(outgoing=True, pattern=r"^!f (.*)")
...
@javes05(outgoing=True, pattern="^.nih$")
async def nih(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`\n(\_/)`"
                     "`\n(•_•)`"
...
@javes05(outgoing=True, pattern="^.hello$")  
async def gtfo(e):
   if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`\n█████████`" 
                     "`\n█▄█████▄█`"    
...
@javes05(outgoing=True, pattern="^.taco$")  
async def taco(e):
   if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("\n{\__/}"
                     "\n(●_●)"
...
@javes05(outgoing=True, pattern="^.paw$")  
async def paw(e):
   if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`(=ↀωↀ=)")


@javes05(outgoing=True, pattern="^.tf$")  
async def tf(e):
   if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
...
@javes05(outgoing=True, pattern="^.bot$")
async def bot(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("` \n   ╲╲╭━━━━╮ \n╭╮┃▆┈┈▆┃╭╮ \n┃╰┫▽▽▽┣╯┃ \n╰━┫△△△┣━╯`"
                     "`\n╲╲┃┈┈┈┈┃  \n╲╲┃┈┏┓┈┃ `")
...
@javes05(outgoing=True, pattern="^.hey$")
async def hey(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("\n┈┈┈╱▔▔▔▔╲┈╭━━━━━\n┈┈▕▂▂▂▂▂▂▏┃HEY!┊😀`"
                     "`\n┈┈▕▔▇▔▔┳▔▏╰┳╮HEY!┊\n┈┈▕╭━╰╯━╮▏━╯╰━━━\n╱▔▔▏▅▅▅▅▕▔▔╲┈┈┈┈`"
...
@javes05(outgoing=True, pattern="^.nou$")
async def nou(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`\n┈╭╮╭╮\n┈┃┃┃┃\n╭┻┗┻┗╮`"
                     "`\n┃┈▋┈▋┃\n┃┈╭▋━╮━╮\n┃┈┈╭╰╯╰╯╮`"
...
@javes05(outgoing=True, pattern="^!logic(?: |$)(.*)")
...
@javes05(outgoing=True, disable_errors=True, pattern="^!whack$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!snake$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!soler$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!think$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!hack$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!bomb(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
...
@javes05(outgoing=True, disable_errors=True, pattern="^!play1$")
async def _(event):
...
@javes05(outgoing=True, disable_errors=True, pattern="^!play2$")
async def _(event):
...
@javes05(outgoing=True, pattern="^!city$")
async def test(event):
    if event.fwd_from:
        return 
    await event.edit("""☁☁🌞      ☁           ☁
...
@javes05(outgoing=True, pattern="^!cheer$")
async def cheer(event):
    if event.fwd_from:
        return
    await event.edit("💐💐😉😊💐💐\n☕ Cheer Up  🍵\n🍂 ✨ )) ✨  🍂\n🍂┃ (( * ┣┓ 🍂\n🍂┃*💗 ┣┛ 🍂 \n🍂┗━━┛  🍂🎂 For YOU  🍰\n💐💐😌😚💐💐")

@javes05(outgoing=True, pattern="^!getwell$")
async def getwell(event):
    if event.fwd_from:
        return
    await event.edit("🌹🌹🌹🌹🌹🌹🌹🌹 \n🌹😷😢😓😷😢💨🌹\n🌹💝💉🍵💊💐💝🌹\n🌹 GetBetter Soon! 🌹\n🌹🌹🌹🌹🌹🌹🌹🌹")

@javes05(outgoing=True, pattern="^!luck$")
async def luck(event):
    if event.fwd_from:
        return
    await event.edit("💚~🍀🍀🍀🍀🍀\n🍀╔╗╔╗╔╗╦╗✨🍀\n🍀║╦║║║║║║👍🍀\n🍀╚╝╚╝╚╝╩╝。 🍀\n🍀・・ⓁⓊⒸⓀ🍀\n🍀🍀🍀 to you💚")

@javes05(outgoing=True, pattern="^!love2$")
...
@javes05(outgoing=True, disable_errors=True, pattern="^!nikal$")
async def _(event):
